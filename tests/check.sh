...
hits=$(bash "$DIR/scan.sh" "$F/all-tics.md" | grep -c '^   [a-z]')
[ "$hits" -eq 38 ] && ok "all 38 patterns fire on all-tics.md" \
                   || no "expected 38 patterns on all-tics.md, got $hits"
...
# 6. rescore.py reproduces the file it ships (needs uv + wordfreq)
